const { STRAPI_URL, readCookie, isSuperAdminUser, friendlyName, avatarUrl } = require('./_shared');
...
module.exports = async (req, res) => {
	...
	try {
		...
		return res.status(200).json({ user: { name: friendlyName(u), email: u.email, avatar: avatarUrl(u), superAdmin: isSuperAdminUser(u) } });
	} catch {
