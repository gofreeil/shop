const { STRAPI_URL, readCookie, isSuperAdminUser, friendlyName } = require('./_shared');
...
module.exports = async (req, res) => {
	...
	try {
		...
		return res.status(200).json({ user: { name: friendlyName(u), email: u.email, superAdmin: isSuperAdminUser(u) } });
	} catch {
